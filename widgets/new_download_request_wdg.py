...
class NewDownloadRequestWdg(BaseRefreshWdg):
    def get_display(self):
        outer_div = DivWdg()
        outer_div.set_id('new-download-request-entry-form')

        page_label = "<div>Fill out the following form to request a new download. The request will be added to the " \
                     "Edel department's list, and will be addressed as soon as possible. You will receive a " \
...
var submit_form = function(values) {
    ...
    server.insert('sthpw/task', new_download_request);

    // Refresh the view
    spt.api.app_busy_hide();
    spt.api.load_tab('New Download Request', 'widgets.NewDownloadRequestWdg');
    spt.info("Your request for a download has been submitted. You will be notified when it is complete.");
}
...
// Get the form values
var containing_element = bvr.src_el.getParent("#new-download-request-entry-form");
